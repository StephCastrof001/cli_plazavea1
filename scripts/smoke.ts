#!/usr/bin/env bun
/**
 * Smoke test e2e — flujo core: search → simulate → add → cart → remove → analytics → track
 * Uso:  bun run smoke
...
const BUN = process.execPath; // ruta real de bun
...
step("simulate stock local", () => {
  if (!skuId) return { ok: false, detail: "sin skuId (search falló)" };
  const r = cli(["simulate", "--sku", skuId]);
  // exit 0 = disponible o sin stock; exit 1 = sesión/dirección faltante
...
step("cart (verify add)", () => {
  ...
  try {
    ...
    // Buscar índice del ítem que acabamos de agregar
...
step("track (add → list → check → remove)", () => {
  ...
  const remove = cli(["track", "remove", skuId]);
