...
async function main() {
  ...
  const quantity = qtyIdx !== -1 ? parseInt(args[qtyIdx + 1] ?? "1") : 1;
  const skuId = args.find((a) => !a.startsWith("--") && a !== args[args.indexOf("--quantity") + 1]);

  if (!skuId) {
    process.stderr.write(chalk.red("Uso: plaza add <skuId> [--quantity N] [--dry-run] [--output json]\n"));
    process.stderr.write(chalk.dim("  Obtén el skuId con: plaza search <término>\n"));
    process.exit(1);
  }

  if (dryRun) {
    process.stdout.write(JSON.stringify({ dryRun: true, skuId, quantity, action: "add_to_cart" }, null, 2) + "\n");
    return;
  }
  ...
  try {
    ...
    if (!addedItem) {
      process.stderr.write(chalk.red("✖ VTEX aceptó el request pero el producto no aparece en el carrito.\n"));
      process.exit(1);
    }
    if (addedItem.availability === "withoutStock") {
      process.stderr.write(chalk.yellow(`⚠ Agregado al carrito, pero sin stock en tu local. Fallará al pagar.\n`));
      process.stderr.write(chalk.dim(`  Verifica: plaza simulate --sku ${skuId} --postal <TU_CP>\n`));
    } else {
