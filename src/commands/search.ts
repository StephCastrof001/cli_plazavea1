...
async function main() {
  ...
  const limit = limitIdx !== -1 ? Number.parseInt(args[limitIdx + 1] ?? "10") : 10;

  const flagValues = new Set<string>();
  for (const flag of ["--output", "--limit"]) {
  ...
  try {
    ...
    for (const p of results) {
      ...
      process.stdout.write(
        `${
          col(p.skuId, 12) + col(p.name, 42) + col(regular, 11) + led.padEnd(p.prices.led ? 11 : 11)
        }  ${oh.padEnd(p.prices.oh ? 11 : 11)}  ${stock}\n`,
      );
    }
