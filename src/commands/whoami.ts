...
import { configExists, configAge, configAgeLabel, CONFIG_PATH } from "../config.js";
...
async function main() {
  ...
  const ttlWarning = age < Infinity && age > 20 * 60 * 1000; // >20 min → puede estar expirada
  ...
  if (ttlWarning) {
    process.stderr.write(chalk.yellow("  ⚠ La sesión puede haber expirado (~30 min TTL). Si hay errores, ejecuta: plaza login\n"));
  }
}
